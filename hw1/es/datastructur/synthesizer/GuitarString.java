...
public class GuitarString {
  ...
  private static void init(BoundedQueue<Double> buffer, int capacity) {
  ...
  public void pluck() {
    ...
    for (int i = 0; i < capacity; i++) {
      while (true) {
        ...
        if (!random.contains(d)) {
            buffer.dequeue();
          buffer.enqueue(getRandomDouble());
          break;
        }
      }
    }
  ...
  public void tic() {
    ...
    double newSample = DECAY * (first + next) * 0.5;
